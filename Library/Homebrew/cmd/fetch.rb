  ...
  def fetch
    ARGV.formulae.each do |f|
      if ARGV.include? "--force" or ARGV.include? "-f"
        ...
        rm_rf where_to unless where_to.empty?
      end
      ...
      unless previous_md5.nil?
        if previous_md5.nil? or previous_md5.empty?
          opoo "Formula has an empty MD5."
        elsif the_tarball.md5 != previous_md5
          opoo "Formula reports different MD5: #{previous_md5}"
        end
      end

      unless previous_sha1.nil?
        if previous_sha1.empty?
          opoo "Formula has an empty SHA1."
        elsif the_tarball.sha1 != previous_sha1
